  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for(let i=0;i<nodes.length;i++) for(let j=i+1;j<nodes.length;j++) {
        ...
        if(d<160) {
          ctx.beginPath()
          ctx.strokeStyle='rgba('+(((i/nodes.length)<0.5)?rgb(color1):rgb(color2))+','+(1-d/160)*0.18+')'
          ctx.lineWidth=.4
      ...
      nodes.forEach((n,i) => {
        ...
        ctx.fillStyle='rgba('+(((i/nodes.length)<0.5)?rgb(color1):rgb(color2))+',0.4)'
        ctx.fill()
      })
        ...
        return (
          <nav style={{
            ...
            <a href="/" style={{display:'flex',alignItems:'center',gap:'6px',textDecoration:'none',flexShrink:0}}
              ...
              <img src="/texasagilabs-logo-new.png" alt="Texas AGI Labs" style={{height:'36px',width:'auto'}} />
